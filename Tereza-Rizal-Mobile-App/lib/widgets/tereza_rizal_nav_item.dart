import 'package:flutter/material.dart';
import '../theme/app_colors.dart';
import '../theme/app_typography.dart';
import 'tereza_rizal_nav_motion.dart';
import 'nav_item_data.dart';

/// One destination cell in [TerezaRizalCurvedNavBar]'s base row. Direct port
/// of Servana's `ServanaNavItem`: the label is ALWAYS shown (not only for
/// the active tab, which is how Tereza, Rizal's old floating pill behaved) and
/// occupies a fixed position, so nothing shifts when selection changes. The
/// icon area above it is a reserved, fixed-height box that goes empty while
/// this tab is active — the active icon is shown instead inside the
/// travelling bubble that floats above the bar, not here.
class TerezaRizalNavItem extends StatelessWidget {
  final NavItemData item;
  final bool isActive;
  final VoidCallback onTap;

  const TerezaRizalNavItem({super.key, required this.item, required this.isActive, required this.onTap});

  static const Color _activeColor = AppColors.brand500;
  static const Color _inactiveColor = AppColors.slate400;

  @override
  Widget build(BuildContext context) {
    final color = isActive ? _activeColor : _inactiveColor;
    return Expanded(
      child: Semantics(
        label: item.label,
        button: true,
        selected: isActive,
        excludeSemantics: true,
        child: InkResponse(
          onTap: onTap,
          containedInkWell: true,
          highlightShape: BoxShape.rectangle,
          radius: 40,
          child: SizedBox(
            height: TerezaRizalNavMotion.barHeight,
            child: Column(
              mainAxisAlignment: MainAxisAlignment.end,
              children: [
                SizedBox(
                  height: TerezaRizalNavMotion.bubbleDiameter * 0.62,
                  child: isActive
                      ? const SizedBox.shrink()
                      : Center(child: Icon(item.outlineIcon, color: color, size: TerezaRizalNavMotion.iconSize)),
                ),
                Padding(
                  padding: const EdgeInsets.only(bottom: 8),
                  child: Text(
                    item.label,
                    maxLines: 1,
                    // The system text scale is deliberately NOT clamped here.
                    // It used to be capped at 1.3x. Measured 2026-08-29 (FE 05)
                    // with the clamp removed, at 280/320/360/412pt:
                    //   - no overflow at any width, at any scale up to 2.0x;
                    //   - the label grows 12pt -> 23pt, which is the whole point
                    //     for a citizen who set large text;
                    //   - "Profile" already ellipsized at 1.3x *with* the clamp,
                    //     so the clamp never bought legibility — it only made
                    //     the same truncated word smaller.
                    // Ellipsis is visual only: the semantics tree still carries
                    // the full label, so a screen reader is unaffected. The icon
                    // above carries the identity regardless.
                    // nav_access_overflow_test.dart holds this at 2.0x.
                    overflow: TextOverflow.ellipsis,
                    style: TextStyle(
                      fontFamily: AppTypography.sans,
                      fontSize: 10.5,
                      height: 1.1,
                      color: color,
                      letterSpacing: -0.3,
                      fontWeight: isActive ? FontWeight.w700 : FontWeight.w500,
                    ),
                  ),
                ),
              ],
            ),
          ),
        ),
      ),
    );
  }
}

/// The travelling active bubble that floats above the bar over whichever
/// slot is selected — direct port of Servana's `_ActiveBubble`.
class TerezaRizalActiveBubble extends StatelessWidget {
  final NavItemData item;
  final bool reduced;

  const TerezaRizalActiveBubble({super.key, required this.item, required this.reduced});

  @override
  Widget build(BuildContext context) {
    return Container(
      width: TerezaRizalNavMotion.bubbleDiameter,
      height: TerezaRizalNavMotion.bubbleDiameter,
      decoration: BoxDecoration(
        color: TerezaRizalNavItem._activeColor,
        shape: BoxShape.circle,
        border: Border.all(color: AppColors.surface, width: 3),
        boxShadow: [
          BoxShadow(color: TerezaRizalNavItem._activeColor.withValues(alpha: 0.30), blurRadius: 10, offset: const Offset(0, 3)),
        ],
      ),
      child: Center(
        child: AnimatedSwitcher(
          duration: reduced ? const Duration(milliseconds: 1) : TerezaRizalNavMotion.selection,
          transitionBuilder: (child, animation) => FadeTransition(opacity: animation, child: child),
          child: Icon(
            item.filledIcon,
            key: ValueKey(item.label),
            color: Colors.white,
            size: TerezaRizalNavMotion.iconSize * TerezaRizalNavMotion.activeIconScale,
          ),
        ),
      ),
    );
  }
}
