...
`grep -rhoE "'tereza_rizal_[a-z_]+'" lib/`. An inventory built to the mandate's number would have
been four keys short — including one holding a photograph.

| # | Key | Contains | Personal? | Encrypted | Erased on sign-out |
|---|---|---|---|---|---|
| 1 | `tereza_rizal_citizen_session` | name, email, mobile, barangay, address, birthdate, sex, civil status, occupation, status | **yes** | no | ✅ |
| 2 | `tereza_rizal_resident_profiles` | full resident profile + **base64 profile photo**, household, family members | **yes — the most sensitive** | no | ✅ |
| 3 | `tereza_rizal_service_requests` | complete request history, purposes, attachments metadata | **yes** | no | ✅ |
| 4 | `tereza_rizal_master_file_documents` | uploaded document records per account | **yes** | no | ✅ |
| 5 | `tereza_rizal_read_notification_ids` | which notifications were read | weakly | no | ✅ |
| 6 | `tereza_rizal_duplicate_alert_resolutions` | duplicate-registration decisions | weakly | no | ✅ |
| 7 | `tereza_rizal_unverified_duplicate_kept_account` | which duplicate account was kept | weakly | no | ✅ |
| 8 | `tereza_rizal_guest_mode` | browsing as guest | no | no | ✅ |
| 9 | `tereza_rizal_balita_posts` | public municipal announcements | **no** | no | ❌ by design — public content, identical for every citizen |
| 10 | `tereza_rizal_onboarding_complete` | has the welcome flow been seen | **no** | no | ❌ by design — device state; erasing it re-shows onboarding to whoever picks the phone up next |
...
2. `tereza_rizal_resident_profiles` still holds a photograph as base64 — the single largest and most
