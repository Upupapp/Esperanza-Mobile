# Tereza, Rizal App — Sign-Up / User Access Process Document

This document describes **the actual behavior of the current codebase** (frontend-only simulation, no real backend — see `TEREZA_RIZAL_MOBILE_WEB_ALIGNMENT.md`). It is a reference for developers and the client, not a redesign proposal. Anything requested previously that isn't in the code yet is explicitly marked **Not yet implemented**.
...
- Persisted to `SharedPreferences` under the key `tereza_rizal_guest_mode` (boolean `true`)
- Any previously stored session (`tereza_rizal_citizen_session`) is removed
...
> **[Feature name]**
> This feature is available to registered Tereza, Rizal users. Create an account or sign in to continue.
>
...
2. **The hamburger drawer** (`tereza_rizal_drawer.dart`) — for a Guest session, the drawer shows `Sign In` and `Create Account` tiles up top instead of Profile/Settings
...
Only one boolean flag: `SharedPreferences['tereza_rizal_guest_mode'] = true`. No name, contact info, or any other guest-specific data is collected or persisted. This flag exists purely so the app remembers "last time you opened the app, you were browsing as a guest" across restarts — it does not identify a person.
...
- Persists the new account under `tereza_rizal_citizen_session`
...
- Explanation: *"Tereza, Rizal LGU is reviewing your submitted information. This usually takes 1–3 business days — no action needed from you right now."*
...
### Status shown
`VerificationStatusPanel` on Profile shows a **"Approved"** `StatusChip` with explanation: *"Your account has been verified. You now have full access to Tereza, Rizal mobile services."* No action button (verified has no `_actionLabel`).
