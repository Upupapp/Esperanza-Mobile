...
Measured directly (`grep -rhoE "'tereza_rizal_[a-z_]+'" lib/`), there are **10**. FE 10's mandate
...
|---|---|---|---|---|
| 1 | `tereza_rizal_citizen_session` | `CitizenSessionService` | JSON object — `CitizenAccount.toJson()` | yes |
| 2 | `tereza_rizal_guest_mode` | `CitizenSessionService` | **bool** (typed, not JSON) | yes |
| 3 | `tereza_rizal_service_requests` | `RequestsService` | JSON array of `ServiceRequest` | yes |
| 4 | `tereza_rizal_balita_posts` | `BalitaService` | JSON array of `Announcement` | yes |
| 5 | `tereza_rizal_master_file_documents` | `MasterFileService` | JSON object, `accountId → List<MasterFileDocument>` | yes |
| 6 | `tereza_rizal_resident_profiles` | `ResidentProfileService` | JSON object, `accountId → ResidentProfile` | yes |
| 7 | `tereza_rizal_read_notification_ids` | `NotificationsService` | JSON array of `String` | yes |
| 8 | `tereza_rizal_duplicate_alert_resolutions` | `NotificationsService` | JSON object, `String → String` | yes |
| 9 | `tereza_rizal_unverified_duplicate_kept_account` | `NotificationsService` | **plain String** (not JSON) | yes |
| 10 | `tereza_rizal_onboarding_complete` | `OnboardingService` | **bool** (typed, not JSON) | yes — see below |
...
|---|---|---|
| `CitizenSessionService` | `tereza_rizal_citizen_session` only | signed out |
| `RequestsService` | `tereza_rizal_service_requests` | empty list |
| `BalitaService` | `tereza_rizal_balita_posts` | empty list |
| `MasterFileService` | `tereza_rizal_master_file_documents` | empty map |
| `ResidentProfileService` | `tereza_rizal_resident_profiles` | empty map |
| `NotificationsService` | all three of its keys | empty |
| `OnboardingService` | `tereza_rizal_onboarding_complete` | not complete |

`CitizenSessionService` deliberately clears **only** the session key — `tereza_rizal_guest_mode` is
