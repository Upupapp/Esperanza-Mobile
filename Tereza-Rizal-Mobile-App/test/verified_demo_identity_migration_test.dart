...
import 'package:tereza_rizal/models/access_level.dart';
import 'package:tereza_rizal/models/citizen_account.dart';
import 'package:tereza_rizal/models/receipt.dart';
import 'package:tereza_rizal/models/service_request.dart';
import 'package:tereza_rizal/screens/profile/digital_id_screen.dart';
...
void main() {
  group('CitizenSessionService migrates a stale persisted account', () {
    testWidgets('a browser signed in as the old verified snapshot now sees Perlita Quiambao', (tester) async {
      SharedPreferences.setMockInitialValues({'tereza_rizal_citizen_session': _staleVerifiedAccountJson});
      ...
      final resaved = jsonDecode(prefs.getString('tereza_rizal_citizen_session')!) as Map<String, dynamic>;
    ...
    testWidgets('a browser signed in as the old duplicate snapshot now sees the duplicate Perlita account', (
      ...
      SharedPreferences.setMockInitialValues({'tereza_rizal_citizen_session': _staleDuplicateAccountJson});
      final session = CitizenSessionService();
      var attempts = 0;
      while (session.loading) {
        attempts++;
    ...
    testWidgets('an account unrelated to the demo identity is left untouched', (tester) async {
      ...
      SharedPreferences.setMockInitialValues({'tereza_rizal_citizen_session': jsonEncode(unrelated.toJson())});
      final session = CitizenSessionService();
      var attempts = 0;
      while (session.loading) {
        attempts++;
  ...
  group('RequestsService migrates stale seeded-demo-request identity', () {
    testWidgets(
      ...
      (tester) async {
        ...
        SharedPreferences.setMockInitialValues({
          'tereza_rizal_service_requests': jsonEncode([staleRequest.toJson()]),
        });
        ...
        final prefs = await SharedPreferences.getInstance();
        final resaved = jsonDecode(prefs.getString('tereza_rizal_service_requests')!) as List;
    ...
    testWidgets('a citizen-submitted (non-demo-seed) request under the old id is never touched by the migration', (
      ...
      SharedPreferences.setMockInitialValues({
        'tereza_rizal_service_requests': jsonEncode([genuineRequest.toJson()]),
      });
  ...
  group('Digital ID', () {
    ...
    testWidgets('Duplicate Perlita (still Pending Review) does not get a second verified Digital ID', (tester) async {
      ...
      expect(find.text('Tereza, Rizal Digital ID'), findsNothing);
