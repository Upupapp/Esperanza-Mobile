// Verifies the Tereza, Rizal-styled explanation dialog shown before any
...
import 'package:tereza_rizal/widgets/requirement_uploader.dart';
...
void main() {
  testWidgets('Take Photo shows the Camera Access Required explanation before anything else', (tester) async {
    ...
    expect(
      find.text('Tereza, Rizal needs access to your camera so you can take a photo and attach it to your request.'),
      findsOneWidget,
    );
  ...
  testWidgets('Choose Image shows the Photo Access Required explanation', (tester) async {
    ...
    expect(
      find.text('Tereza, Rizal needs access to your photos so you can select an image to attach to your request.'),
      findsOneWidget,
    );
    expect(find.text('Not Now'), findsOneWidget);
    expect(find.text('Continue'), findsOneWidget);
  ...
  testWidgets('Choose File / PDF shows the Document Access explanation, never a broad storage warning', (tester) async {
    ...
    expect(
      find.text('Tereza, Rizal needs permission to let you choose a document to attach to your request.'),
      findsOneWidget,
    );
  ...
  testWidgets('the explanation dialog uses the existing Tereza, Rizal confirm-sheet styling (rounded sheet, primary/secondary buttons)', (tester) async {
    await _openPickerSheet(tester);
    await tester.tap(find.text('Choose File / PDF'));
    await tester.pumpAndSettle();

    // Same bottom-sheet shell every other Tereza, Rizal confirmation uses
