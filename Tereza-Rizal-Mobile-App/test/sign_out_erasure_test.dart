...
import 'package:tereza_rizal/services/sign_out.dart';
  ...
  group('Signing out erases the account from the device', () {
    ...
    Future<void> setUpSignedIn(WidgetTester tester) async {
      SharedPreferences.setMockInitialValues({
        ...
        'tereza_rizal_service_requests': jsonEncode([_request().toJson()]),
        'tereza_rizal_read_notification_ids': jsonEncode(['notif-1', 'notif-2']),
        'tereza_rizal_duplicate_alert_resolutions': jsonEncode({'scenario-a': 'kept'}),
        'tereza_rizal_onboarding_complete': true,
      });
    ...
    testWidgets('the request history is gone from disk, not just from the screen', (tester) async {
      ...
      await prefs.reload();
      final raw = prefs.getString('tereza_rizal_service_requests');
      expect(
    ...
    testWidgets('notification bookkeeping is cleared', (tester) async {
      ...
      final prefs = await SharedPreferences.getInstance();
      await prefs.reload();
      expect(prefs.getString('tereza_rizal_read_notification_ids'), isNull);
      expect(prefs.getString('tereza_rizal_duplicate_alert_resolutions'), isNull);
      expect(prefs.getString('tereza_rizal_unverified_duplicate_kept_account'), isNull);
    });
    ...
    testWidgets('the session and guest flag are cleared too', (tester) async {
      ...
      final prefs = await SharedPreferences.getInstance();
      await prefs.reload();
      expect(prefs.getString('tereza_rizal_citizen_session'), isNull);
      expect(prefs.getBool('tereza_rizal_guest_mode'), isNull);
    });
    ...
    testWidgets('onboarding completion survives — it is device state, not personal data', (tester) async {
      ...
      final prefs = await SharedPreferences.getInstance();
      await prefs.reload();
      expect(prefs.getBool('tereza_rizal_onboarding_complete'), isTrue);
    });
