...
class PersistenceRecovery {
  ...
  static List<T> decodeEach<T>(
    ...
    for (final entry in source) {
      try {
        ...
        skipped++;
        developer.log('Skipping unreadable $what entry: $error', name: 'tereza_rizal.persistence', error: error);
      }
    }
    if (skipped > 0) {
      debugPrint('[tereza_rizal.persistence] skipped $skipped of ${skipped + decoded.length} $what entries');
    }
    return decoded;
  }
  ...
  static Map<String, V> decodeEntries<V>(
    ...
    source.forEach((key, value) {
      try {
        ...
        skipped++;
        developer.log('Skipping unreadable $what "$key": $error', name: 'tereza_rizal.persistence', error: error);
      }
    });
    if (skipped > 0) {
      debugPrint('[tereza_rizal.persistence] skipped $skipped of ${skipped + decoded.length} $what entries');
  ...
  static Future<void> discardUnreadable({
    ...
    developer.log(
      ...
      name: 'tereza_rizal.persistence',
      error: error,
    );
    debugPrint('[tereza_rizal.persistence] $record');
