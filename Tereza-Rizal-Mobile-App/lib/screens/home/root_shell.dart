...
import '../../widgets/tereza_rizal_curved_navbar.dart';
...
/// navigation (see widgets/tereza_rizal_curved_navbar.dart's doc comment for
...
/// Profile lives in the hamburger drawer (see widgets/tereza_rizal_drawer.dart)
...
/// [_items] and what [TerezaRizalCurvedNavBar] shows as its active slot) and
...
class RootShell extends StatefulWidget {
  ...
  /// Emergency=3, matching [TerezaRizalCurvedNavBar]'s slot order.
...
class _RootShellState extends State<RootShell> {
  ...
  /// duplicate account (Phase 6 — see MockCatalog.duplicateVerifiedDemoAccount)
  ...
  void openService(ServiceLauncherTarget target) {
    ...
    if (session.account?.id == MockCatalog.duplicateVerifiedDemoAccount.id) {
  ...
  /// the real, verified Perlita and lands on Home — deliberately not
  ...
  Future<void> _promptSwitchToVerifiedAccount() async {
    final switchAccount = await AppDialogs.confirm(
      ...
      message:
          'This account is a duplicate of an existing verified Tereza, Rizal account. You can continue using this '
    ...
    if (mounted) AppDialogs.toast(context, 'Switched to your verified Tereza, Rizal account.');
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      // TerezaRizalCurvedNavBar reports a bounding box that is its own bar
      // height only (matching Servana's own SizedBox — see
      // widgets/tereza_rizal_curved_navbar.dart); the raised bubble and "+"
