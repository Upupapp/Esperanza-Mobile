// Verifies the "moving navbar shape" requirement for TerezaRizalCurvedNavBar —
// a direct port of the Servana Client App's curved main navigation (see
// lib/widgets/tereza_rizal_curved_navbar.dart's doc comment): the travelling
...
import 'package:tereza_rizal/widgets/tereza_rizal_center_action.dart';
import 'package:tereza_rizal/widgets/tereza_rizal_curved_navbar.dart';
import 'package:tereza_rizal/widgets/tereza_rizal_curved_nav_painter.dart';
import 'package:tereza_rizal/widgets/tereza_rizal_nav_item.dart';
import 'package:tereza_rizal/widgets/tereza_rizal_nav_motion.dart';
import 'package:tereza_rizal/widgets/nav_item_data.dart';
import 'package:tereza_rizal/widgets/service_launcher_menu.dart';
...
/// A tiny stateful harness so tests can drive [TerezaRizalCurvedNavBar] the
...
class _HarnessState extends State<_Harness> {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        body: const SizedBox(),
        bottomNavigationBar: TerezaRizalCurvedNavBar(
          items: _items,
          activeIndex: index,
...
double _indicatorLeft(WidgetTester tester) {
  return tester.getTopLeft(find.byType(TerezaRizalActiveBubble)).dx;
}

double _cradleCentreX(WidgetTester tester) {
  final paint = tester.widget<CustomPaint>(find.byKey(const ValueKey('nav-bar-shape')));
  return (paint.painter as TerezaRizalCurvedNavPainter).cradleCentreX;
}
...
/// derived the same way TerezaRizalCurvedNavBar itself computes it — five
...
double _expectedRestingLeft(double screenWidth, int itemIndex) {
  ...
  final minCentre = TerezaRizalNavMotion.bubbleDiameter / 2 + TerezaRizalNavMotion.surfaceRadius * 0.5;
  final rawCentreX = (slot + 0.5) * slotWidth;
  final centreX = rawCentreX.clamp(minCentre, screenWidth - minCentre);
  return centreX - TerezaRizalNavMotion.bubbleDiameter / 2;
}
...
void main() {
  ...
  testWidgets('the cradle and the floating bubble are driven by the same center at rest and mid-flight', (tester) async {
    ...
    // At rest.
    final restBubbleCenter = _indicatorLeft(tester) + TerezaRizalNavMotion.bubbleDiameter / 2;
    ...
    final midBubbleCenter = _indicatorLeft(tester) + TerezaRizalNavMotion.bubbleDiameter / 2;
  ...
  for (final t in transitions) {
    ...
    testWidgets('$from -> $to: bubble and cradle both settle centered above the destination tab', (tester) async {
      ...
      final screenWidth = tester.getSize(find.byType(TerezaRizalCurvedNavBar)).width;
      ...
      expect(_cradleCentreX(tester), closeTo(settledLeft + TerezaRizalNavMotion.bubbleDiameter / 2, 0.5));
  ...
  testWidgets('interrupted mid-flight tap (Home -> Emergency -> Balita) redirects smoothly and settles correctly', (tester) async {
    ...
    expect(_cradleCentreX(tester), closeTo(expectedLeft + TerezaRizalNavMotion.bubbleDiameter / 2, 0.5));
  });
  ...
  testWidgets('opening the center launcher hides the tab bubble and pulls the cradle to the center slot', (tester) async {
    ...
    final center = tester.widget<TerezaRizalCenterAction>(find.byType(TerezaRizalCenterAction));
    ...
    await tester.pumpAndSettle();
    expect(find.byKey(const ValueKey('nav-floating-indicator')), findsOneWidget);
    final restedCenter = tester.widget<TerezaRizalCenterAction>(find.byType(TerezaRizalCenterAction));
