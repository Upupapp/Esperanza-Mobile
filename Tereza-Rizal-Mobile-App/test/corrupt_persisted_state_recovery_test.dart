...
import 'package:tereza_rizal/models/service_request.dart';
import 'package:tereza_rizal/services/balita_service.dart';
import 'package:tereza_rizal/services/citizen_session_service.dart';
import 'package:tereza_rizal/services/master_file_service.dart';
import 'package:tereza_rizal/services/notifications_service.dart';
import 'package:tereza_rizal/services/onboarding_service.dart';
import 'package:tereza_rizal/services/persistence_recovery.dart';
import 'package:tereza_rizal/services/requests_service.dart';
import 'package:tereza_rizal/services/resident_profile_service.dart';
...
void main() {
  ...
  group('A corrupt or incompatible persisted payload never bricks the splash', () {
    testWidgets('CitizenSessionService falls back to signed-out', (tester) async {
      SharedPreferences.setMockInitialValues({'tereza_rizal_citizen_session': _notJson});
    ...
    testWidgets('CitizenSessionService clears only the session key, not the guest flag', (tester) async {
      ...
      SharedPreferences.setMockInitialValues({
        'tereza_rizal_citizen_session': _notJson,
        'tereza_rizal_guest_mode': true,
      });
      ...
      await prefs.reload();
      expect(prefs.getString('tereza_rizal_citizen_session'), isNull, reason: 'the unreadable key must go');
      expect(prefs.getBool('tereza_rizal_guest_mode'), isTrue, reason: 'the readable neighbour must survive');
    });
    ...
    testWidgets('the discard is logged, never silent', (tester) async {
      ...
      expect(PersistenceRecovery.discards.single.keys, ['tereza_rizal_citizen_session']);
    });

    testWidgets('RequestsService drops an unreadable list rather than hanging', (tester) async {
      SharedPreferences.setMockInitialValues({'tereza_rizal_service_requests': _wrongType});
    ...
    testWidgets('BalitaService survives a non-JSON payload', (tester) async {
      SharedPreferences.setMockInitialValues({'tereza_rizal_balita_posts': _notJson});
    ...
    testWidgets('MasterFileService survives a payload of the wrong type', (tester) async {
      SharedPreferences.setMockInitialValues({'tereza_rizal_master_file_documents': '["a list, not a map"]'});
    ...
    testWidgets('NotificationsService survives a non-JSON payload', (tester) async {
      SharedPreferences.setMockInitialValues({
        'tereza_rizal_read_notification_ids': _notJson,
        'tereza_rizal_duplicate_alert_resolutions': _notJson,
      });
    ...
    testWidgets('ResidentProfileService survives a payload of the wrong type', (tester) async {
      SharedPreferences.setMockInitialValues({'tereza_rizal_resident_profiles': _wrongType});
  ...
  group('The splash reads a typed preference, and that can throw too', () {
    ...
    testWidgets('a non-bool onboarding flag does not throw out of isComplete', (tester) async {
      SharedPreferences.setMockInitialValues({'tereza_rizal_onboarding_complete': 'yes'});
    ...
    testWidgets('the unreadable flag is cleared so the next launch is clean', (tester) async {
      ...
      await tester.runAsync(() async {
        ...
        final prefs = await SharedPreferences.getInstance();
        await prefs.reload();
        expect(prefs.get('tereza_rizal_onboarding_complete'), isNull);
      });
    });
  });
  ...
  group('An enum value this build no longer knows decodes to a fallback, not a throw', () {
    ...
    testWidgets('an unknown ServiceCategory keeps the rest of the request readable', (tester) async {
      ...
      SharedPreferences.setMockInitialValues({
        'tereza_rizal_service_requests': jsonEncode([json]),
      });
