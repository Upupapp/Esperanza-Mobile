...
void main() {
    ...
    (tester) async {
      ...
      expect(
        find.text(
          ...
          'but it cannot be upgraded to Verified status because Tereza, Rizal follows a one-person, '
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(find.text('Switched to your verified Tereza, Rizal account.'), findsOneWidget);
  ...
  testWidgets('Duplicate account: "Not Now" on the verified-account popup leaves the session and screen unchanged', (
    ...
    expect(find.text('Switched to your verified Tereza, Rizal account.'), findsNothing);
    expect(tester.takeException(), isNull);
  });
}
