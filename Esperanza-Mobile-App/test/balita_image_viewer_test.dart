...
/// assertions.
final _mangrovePostImage = find.byWidgetPredicate(
  (w) => w is Image && w.image is AssetImage && (w.image as AssetImage).assetName == 'assets/images/News page section.png',
);
...
void main() {
  testWidgets('Guest: can open the viewer and read the post, but React/Comment/Share all show the account-required notice', (tester) async {
  ...
  testWidgets('Ronaldo Bautista (signed in, unverified): not treated as Guest — can React in the viewer, state syncs back to the feed', (tester) async {
    SharedPreferences.setMockInitialValues({'esperanza_onboarding_complete': true});
    _setPhoneViewport(tester);
    await tester.pumpWidget(const EsperanzaMobileApp());
    await tester.pumpAndSettle();
