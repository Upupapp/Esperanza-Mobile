...
The app's icon is the **Tereza, Rizal seal**. It is generated, never hand-edited.

```sh
dart run flutter_launcher_icons        # from Tereza-Rizal-Mobile-App/
```
...
Both are composed from `assets/images/Logo/tereza-rizal-seal.png` — the seal the app
ships at runtime. They are **build-time sources only** and are deliberately
not declared in `pubspec.yaml`'s `assets:`, so they do not ship a second copy.

| File | Canvas | Seal size | Background |
|---|---|---|---|
| `tereza_rizal_icon.png` | 1024×1024 | 84 % | opaque white |
| `tereza_rizal_icon_foreground.png` | 1024×1024 | 84 % | transparent |

### Why white, not the app's navy

The seal is drawn for a light ground: a black outer ring around a gold band. On `#0B1730`
navy the black ring disappears into the background. White keeps the ring visible.
