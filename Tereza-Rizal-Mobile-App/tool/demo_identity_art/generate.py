"""Generate the demo identities' profile avatars and government-ID card images.
    ...
    python tool/demo_identity_art/generate.py        # from Tereza-Rizal-Mobile-App/
...
IDENTITIES = [
    ...
    ('Perlita Quiambao', 'Perlita Profile.png', 'PERLITA DEMO ID.png',
     ...
     'Purok 2, Brgy. Baras, Tereza, Rizal', (1578, 997)),
    ('Nicanor Sarmiento', 'Nicanor Sarmiento.png', 'NICANOR ID DEMO.png',
     'Tereza, Rizal Resident ID', 'ESP-RES-2024-9001', 'June 8, 1990',
     'Purok 2, Brgy. Labangtaytay, Tereza, Rizal', (1578, 997)),
    ('Anacleto Dimaculangan', 'Anacleto Dimaculangan.png', 'ANACLETO ID DEMO.png',
     'Tereza, Rizal Resident ID', 'ESP-RES-2024-9013', 'October 27, 1992',
     'Purok 3, Brgy. Libertad, Tereza, Rizal', (1573, 1000)),
]
...
def wallet_card(title, subtitle, name, id_number, fields, path, size, back=False):
    ...
    d.text((int(w * 0.03), int(h * 0.030)), 'MUNICIPALITY OF TEREZA, RIZAL', font=font(True, int(h * 0.036)), fill=SLATE_400)
...
WALLET = [
    ...
    ('Barangay Resident ID', 'REVERSE', 'Perlita Quiambao', 'ESP-RES-2024-9002',
     [('ISSUED BY', 'Municipality of Tereza, Rizal'), ('VALIDITY', 'No expiry — demonstration card'),
...
def main():
    if not os.path.isdir(OUT):
        raise SystemExit('run this from Tereza-Rizal-Mobile-App/ — %s not found' % OUT)
