...
```
Building ph.gov.tereza.terezaRizal for simulator (ios)...
...
```
flutter: [tereza_rizal.persistence] CitizenSessionService discarded tereza_rizal_citizen_session:
...
`flutter.tereza_rizal_citizen_session` as a key path. `PlistBuddy` writes a value that reads back as
