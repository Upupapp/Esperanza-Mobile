...
// TerezaRizalReceipt design shared by GCash/Maya/Onsite/Free (only the "Mode
...
import 'package:tereza_rizal/screens/shared/receipt_screen.dart';
...
import 'package:tereza_rizal/utils/receipt_export.dart';
import 'package:tereza_rizal/widgets/app_button.dart';
import 'package:tereza_rizal/widgets/receipts/tereza_rizal_receipt.dart';
...
void main() {
  ...
  group('View Receipt', () {
    testWidgets('Onsite payment shows the unified Tereza, Rizal receipt, relabeled Due Onsite (not a paid amount)', (
      tester,
    ) async {
      final requests = await _readyRequests(tester);
      ...
      expect(find.byType(TerezaRizalReceipt), findsOneWidget);
      expect(find.text('Municipality of Tereza, Rizal'), findsOneWidget);
    ...
    testWidgets('GCash payment shows the exact same unified Tereza, Rizal receipt, PAID, with GCash as its Mode of Payment', (
      tester,
    ) async {
      final requests = await _readyRequests(tester);
      final request = await _submitPaidBarangayClearance(requests, 'GCash');
      ...
      // overflow from its own source size (see tereza_rizal_receipt.dart).
      expect(find.byType(SvgPicture), findsOneWidget);
      expect(tester.takeException(), isNull);
    });

    testWidgets('Maya payment shows the exact same unified Tereza, Rizal receipt, PAID, with Maya as its Mode of Payment', (
      tester,
    ) async {
      final requests = await _readyRequests(tester);
      ...
      // it can't overflow or dominate the row — see tereza_rizal_receipt.dart.
  ...
  group('Download Receipt', () {
    ...
    testWidgets('captureRepaintBoundary produces non-empty, well-formed PNG bytes from a rendered receipt', (
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: RepaintBoundary(key: key, child: TerezaRizalReceipt(receipt: receipt)),
          ),
        ),
      );
      await tester.pumpAndSettle();
